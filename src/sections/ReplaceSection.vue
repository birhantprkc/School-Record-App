  ...
  <div class="flex flex-col h-full overflow-hidden box-border">
    ...
    >
        ...
        <div class="bg-surface border border-line rounded-[10px] p-4">
          ...
          <div v-if="showAddForm" class="flex items-center gap-2 flex-wrap p-3 bg-raised rounded-lg mb-2.5">
            ...
            <input
                ...
                @keydown.enter="submitAdd"
            />
            <label class="flex items-center gap-1 text-base text-ink-5 whitespace-nowrap cursor-pointer shrink-0">
            ...
            <p v-if="addError" class="text-red text-base m-0 mt-1 w-full">{{ addError }}</p>
          </div>
          ...
          <!-- 규칙 테이블 -->
          <div v-if="ruleStore.rules.length === 0 && !ruleStore.loading" class="text-ink-4 text-base py-3.5 text-center">
            규칙이 없습니다. 추가 버튼으로 새 규칙을 만드세요.
          </div>

          <div v-else class="flex flex-col gap-0.5">
            <!-- 헤더 행 --><!-- table header: 규칙 목록 컬럼 헤더 -->
            ...
            >
              ...
              <template v-if="editingId === rule.id">
                ...
                <p v-if="editError" class="text-red text-base m-0 mt-1 w-full col-span-full">{{ editError }}</p>
              </template>
              ...
              <template v-else>
                ...
                <div class="flex flex-row items-center justify-center gap-1">
                  ...
                  <span class="text-base text-ink-5 min-w-[26px] text-center tabular-nums">{{ rule.priority }}</span>
                  <button
                      class="flex items-center justify-center w-5 h-5 rounded-[3px] border-none bg-transparent text-ink-5 cursor-pointer hover:bg-raised hover:text-ink-4"
                ...
                <div class="flex items-center gap-1.5 overflow-hidden justify-end">
                  <span v-if="rule.is_regex"
                        class="inline-block text-xs py-[1px] px-[5px] rounded-[4px] bg-blue/20 text-blue-2 font-semibold shrink-0 mr-1">정규식</span><!-- badge: 정규식 표시 뱃지 -->
      ...
      <div v-if="step === 2">
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
          ...
          >
            ...
            <span class="text-base text-ink-5">모든 생기부 기록에 치환을 적용합니다</span>
          </div>
          <div
              class="relative border-2 rounded-[10px] py-[18px] px-5 cursor-pointer flex flex-col gap-1.5 transition-[border-color,background-color] duration-200"
          ...
          >
            ...
            <span class="text-base text-ink-5">치환을 적용할 영역을 직접 선택합니다</span>
          </div>
        </div>
        ...
        >
          <p class="text-base text-ink-5 m-0 mb-3">치환을 적용할 영역 선택</p>
      ...
      <div v-if="step === 3">
        ...
        <!-- 스냅샷 권장 안내 -->
        <div class="flex items-center gap-1.5 text-amber text-base mt-3 mb-3 py-[9px] px-3 bg-amber/[0.06] rounded-[6px] border border-amber/15">
          <TriangleAlert :size="14"/>
        ...
        <div v-if="previewItems.length > 0" class="bg-surface border border-line rounded-[10px] p-4">
          <div class="flex items-center justify-between mb-3">
            <span class="text-lg font-semibold text-ink-4 uppercase tracking-[0.05em]">
              ...
              <span v-if="previewItems.length > PREVIEW_LIMIT && !showAllPreview" class="text-ink-5 text-base font-normal normal-case tracking-normal">
                ({{ PREVIEW_LIMIT }}건만 표시)
              </span>
            </span>
          </div>
          ...
          <div v-for="item in visiblePreviewItems" :key="`${item.activity_id}-${item.student_id}`"
            ...
            <div class="flex items-center gap-1.5 mb-1.5">
              <span class="text-base text-ink-5">{{ item.student_name }}</span>
              <span class="text-ink-5">/</span>
              <span class="text-base text-ink-5">{{ item.activity_name }}</span>
            </div>
          ...
          <button
              ...
              class="inline-flex items-center gap-[5px] py-1.5 px-3 rounded-[6px] border border-line bg-transparent text-ink-5 text-base cursor-pointer transition-colors hover:bg-line hover:text-ink-4 mt-3"
        ...
        <div v-else-if="hasRanPreview && previewItems.length === 0 && applyResult === null && !previewError"
          ...
          <p class="text-base text-ink-5 m-0">활성화된 규칙과 일치하는 텍스트를 찾지 못했습니다.</p>
        </div>

        <!-- 초기 안내 -->
        <div v-else-if="!hasRanPreview && !isPreviewing && applyResult === null && !previewError"
             class="text-ink-4 text-base py-9 px-[18px] text-center border border-dashed border-line rounded-lg">
        ...
        <div v-if="applyResult" class="flex flex-col items-center gap-4 py-12">
          ...
          <div class="flex gap-8">
            <div class="flex flex-col items-center gap-1">
              ...
              <span class="text-base text-ink-5">(전체 {{ applyResult.total_count }}건 중)</span>
            </div>
          </div>
