...
- **APP_CONFIGS를 읽는 코드가 열기 흐름에 있다면 `has_app_configs`를 먼저 통과해야 한다.** 0.2.5까지의 릴리즈로 만든 파일에는 그 테이블이 없고 `open_existing`은 `schema.sql`을 실행하지 않으므로 끝내 생기지 않는다. 하나라도 빠뜨리면 `no such table`로 그 파일은 열리지도 않는다 — 마이그레이션이 그런 파일을 정식 지원해도 거기에 닿지 못한다.
