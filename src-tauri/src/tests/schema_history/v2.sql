...
-- 스키마 버전 2 스냅샷
--
-- DDL은 v1과 동일하다. 이 버전이 나타내는 것은 **데이터 표현 변경**이다 —
-- ActivityRecordHistory.note와 Snapshot.memo가 암호화 대상에 포함됐다
-- (commands/crypto.rs의 ENCRYPTED_COLUMNS 참고). 테이블 모양이 같아도 어느 파일이
-- 이미 변환됐는지 구분할 표식이 필요해 user_version을 올렸다.
--
-- ⚠️ 이 파일은 수정 금지. 이미 배포된 DB 파일의 구조를 기록한 사본이다.
-- ================================================================
